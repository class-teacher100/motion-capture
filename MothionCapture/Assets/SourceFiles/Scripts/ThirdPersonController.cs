...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        [Header("Pose Locomotion")]
        [Tooltip("When true, root motion is driven by PoseForward/PoseTurn (step-in-place + body turn) instead of camera-relative WASD. Set by PoseInputReceiver.")]
        public bool PoseLocomotion = false;

        [Tooltip("How fast the character yaws when PoseTurn is at full deflection (deg/sec)")]
        public float TurnSpeed = 120.0f;

        // Set every frame by PoseInputReceiver in pose mode.
        [HideInInspector] public float PoseForward = 0.0f;  // 0..1, forward speed scale
        [HideInInspector] public float PoseTurn = 0.0f;     // -1..1, yaw rate scale
        ...
        private void CameraRotation()
{
    ...
    CinemachineCameraTarget.transform.rotation = Quaternion.Euler(
        ...
        {
            // Yaw the character from upper-body turn intent.
            if (Mathf.Abs(PoseTurn) > _threshold)
                transform.Rotate(0.0f, PoseTurn * TurnSpeed * Time.deltaTime, 0.0f);

            // Forward speed from step-in-place cadence (no sprint in pose mode).
            float targetSpeed = PoseForward < _threshold ? 0.0f : PoseForward * MoveSpeed;

            float currentHorizontalSpeed = new Vector3(_controller.velocity.x, 0.0f, _controller.velocity.z).magnitude;
            float speedOffset = 0.1f;

            // accelerate or decelerate to target speed (same easing as keyboard Move)
            if (currentHorizontalSpeed < targetSpeed - speedOffset ||
                currentHorizontalSpeed > targetSpeed + speedOffset)
            {
            ...
            // Move along the character's own facing direction (plus gravity).
            Vector3 targetDirection = transform.forward;
            _controller.Move(targetDirection.normalized * (_speed * Time.deltaTime) +
                             new Vector3(0.0f, _verticalVelocity, 0.0f) * Time.deltaTime);

            // Keep locomotion animation idle; the body is driven by PoseAvatarDriver.
            if (_hasAnimator)
            {
