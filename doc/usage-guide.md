...
- Unity 6000.4.7f1
- Python 3.12 + uv（推論は MediaPipe で CPU 動作、GPU 不要）
...
初回起動時に MediaPipe Pose モデルが自動ダウンロードされます。
...
MediaPipe の 3D ランドマークを使うため、奥行き方向の動き（前後の腕振り等）や体のひねりも
反映されます。それでも単眼カメラの奥行きは推定値のため、**カメラに正対するほど安定**します。
奥行きの向きが逆に見える場合は `PoseAvatarDriver` の `depthScale` の符号を反転してください。
...
| 前後（奥行き）方向が逆 | **Depth Scale** の符号を反転（既定 -1 → +1） |
| 奥行きを使いたくない（平面ミラーに戻す） | **Use Depth** をオフ |
...
| `Use Depth` | true | 3D ワールド座標(`kp3d`)で奥行きを反映。オフで平面ミラーに戻る |
| `Depth Scale` | -1 | 奥行き軸の符号・倍率。前後が逆なら符号反転、影響を弱めるなら小さく |
...
MediaPipe は CPU で動作します。`main.py` の `MODEL_VARIANT` を `"full"`→`"lite"` にすると
速くなり、`"heavy"` にすると精度が上がる代わりに重くなります。負荷が高い場合はカメラ解像度
（`FRAME_W` / `FRAME_H`）を下げてください。
